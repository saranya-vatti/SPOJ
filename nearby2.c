...
int main() {
    ...
    for(i=0;i<n;i++) {
        ...
        /*printf("\ndistArr:\n");
        for(j=0;j<NUM_T;j++) {
            printf("Distance of (%lf,%lf) to (%lf,%lf) is %lf\n",qx,qy,tx[j],ty[j],distArr[j]);
        }*/
        for(j=1;j<=NUM_T-1;j++) {
            k=j;
            while(k>0 && (distArr[k]-distArr[k-1]<0.001)) {
                /*printf("distArr[%d] = %lf\n",k,distArr[k]);
                printf("distArr[%d] = %lf\n",k-1,distArr[k-1]);
                printf("distArr[%d]-distArr[%d] = %lf\n",k,k-1,distArr[k]-distArr[k-1]);*/
                if(distArr[k]-distArr[k-1]<0.001 || (distArr[k]-distArr[k-1]==0.001 && indArr[k-1]>indArr[k])) {
